...
| 状态标识                | 状态描述                                                                 |
|-------------------------|--------------------------------------------------------------------------|
| READ/WRITE (Private)    | 页面可读写，仅被单个进程独占（无共享）。                                 |
| READ-ONLY (Shared COW)  | 页面只读，被多个进程共享（`fork()`后父子进程的初始状态）。               |
...
└─────────────────┘                └─────────────────┘
```
Copy-on-Write（COW）运行机制说明

在 ucore 操作系统中，Copy-on-Write（COW）机制用于优化进程创建（fork）时的内存管理开销，其运行过程可分为以下三个阶段：

1. fork 阶段（建立共享只读映射）

当父进程调用 fork() 创建子进程时，内核并不立即复制父进程的用户空间物理页面。相反，父子进程的页表项同时映射到同一物理页，并将这些页表项的写权限清除，同时设置 Copy-on-Write 标志。此时，物理页的引用计数递增，表明该页被多个进程共享。

2. 正常执行阶段（读操作）

在 COW 状态下，父子进程均可对共享页面执行读操作。由于页表项仍然有效且具有读权限，读访问不会触发异常，系统行为与普通内存访问完全一致。

3. 写时复制阶段（写操作触发 page fault）

当任一进程尝试对 COW 页面执行写操作时，由于页表项不具备写权限，处理器触发写页错误（page fault）。内核在异常处理过程中检测到该页带有 COW 标志，并根据物理页的引用计数采取不同策略：

若物理页仍被多个进程共享，则内核分配一个新的物理页，将原页面内容复制到新页，并更新当前进程页表项，使其指向新页并恢复写权限；

若物理页仅被当前进程引用，则无需复制，仅需恢复该页的写权限并清除 COW 标志。

随后，内核刷新对应的 TLB 项，进程继续执行写操作。该修改仅对当前进程可见，从而保证进程间的内存隔离性。
